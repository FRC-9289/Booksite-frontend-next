export default async function createGradeConfig(grade: string, maleRooms: number[], femaleRooms: number[]) {
  console.log("Creating grade config for grade:", grade, "maleRooms:", maleRooms, "femaleRooms:", femaleRooms);

  const res = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/create-grade-config`, {
    method: "POST",
    headers: {
